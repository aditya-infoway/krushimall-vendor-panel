...
export default function Subcategory() {
 ...
 const [filterSubCategories, setFilterSubCategories] = useState<SubCategoryType[]>([]);
...
const [confirmState, setConfirmState] = useState<"pending" | "success" | "error">("pending");
  ...
  const handleOpenEditDrawer = (item: SubCategoryType) => {
    setEditId(item.id);
    reset({
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="dark:text-dark-300 mt-1 text-sm text-gray-500">
            Manage all subcategories from here
          </p>
        </div>
              ...
              <Combobox
                ...
                onChange={(opt: any) => {
                  ...
                  const categorySubCategories = subCategories.filter(
                    (s) => String(s.categoryId) === String(opt.id),
                  );
                  setFilterSubCategories(categorySubCategories);
                }}
                      ...
                      <div className="dark:border-dark-500 flex h-15 w-15 items-center justify-center overflow-hidden rounded-lg border border-gray-200 bg-gray-50">
                        {item.image ? (
                          ...
                          <span className="text-xs font-bold text-gray-400 uppercase">
                            {item.subCategoryName.substring(0, 2)}
                          </span>
                        )}
                      </div>
                    </Td>
              ...
              >
                <div className="dark:border-dark-500 flex items-center justify-between border-b border-gray-200 px-5 py-4">
                  <h2 className="dark:text-dark-50 text-lg font-semibold text-gray-800">
                    {editId !== null ? "Edit Subcategory" : "Add Subcategory"}
                  </h2>
                  <Button
                ...
                <div className="grow space-y-5 overflow-y-auto p-5">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      {...register(
                        "subCategoryName",
                        formValidationRules.subCategoryName,
                      )}
                      error={
        ...
        messages={{
          pending: {
            ...
            description: isBulkDelete
              ? `Are you sure you want to delete ${selectedIds.length} selected subcategories? This action cannot be undone.`
              : "Are you sure you want to delete this subcategory? Once deleted, it cannot be restored.",
            actionText: isBulkDelete ? "Delete All" : "Delete",
          },
          success: {
            title: "Deleted Successfully",
            description: isBulkDelete
              ? `${selectedIds.length} subcategories have been deleted.`
