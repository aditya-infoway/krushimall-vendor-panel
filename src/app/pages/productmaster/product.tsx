...
export default function ProductList() {
  ...
  const getProducts = async () => {
    try {
      setLoading(true);
      const response = await apiHelper.get("/vendor-panel/product");
      let data = response?.data || response;
      if (!Array.isArray(data)) data = [];

      const mappedData: ProductType[] = data.map((item: any) => ({
        id: item.id || item._id,
        image: apiHelper.getImageUrl(item.mainImage || item.image),
        productName: item.productName || item.name || "",
        sku: item.sku || "-",
        category: item.category?.categoryName || item.category?.name || "",
        categoryId: item.category?.id || item.categoryId,
        subCategory:
          item.subCategory?.subCategoryName || item.subCategory?.name || "",
  ...
  const getCategories = async () => {
    try {
      const response = await apiHelper.get("/vendor-panel/category");
  ...
  const statusFilterOptions = [
    { id: "All", name: "All Verification Statuses" },
  ...
  const handleAddProduct = () => {
    navigate("/productmaster/add-product");
  };

  const handleEditProduct = (id: number) => {
    navigate(`/productmaster/edit-product/${id}`);
  };
  ...
  const performDelete = async () => {
    ...
    try {
      if (isBulkDelete) {
        await Promise.all(
          selectedIds.map((id) =>
            apiHelper.delete(`/vendor-panel/product/${id}`),
          ),
        );
        ...
        await apiHelper.delete(`/vendor-panel/product/${deleteTargetId}`);
  ...
  const handleToggleVerification = async (id: number) => {
    ...
    try {
      const newVerificationStatus =
        item.verificationStatus === "APPROVED" ? "PENDING" : "APPROVED";

      await apiHelper.put(`/vendor-panel/product/${id}`, {
  ...
  const filteredData = products.filter((item) => {
    ...
    const matchesStatus =
      ...
      item.verificationStatus === selectedStatusFilter;
    ...
    <div className="relative min-h-screen space-y-6 p-4 pb-28 text-gray-900 md:p-6 dark:text-gray-100">
      ...
      <div className="dark:bg-dark-800 dark:border-dark-700 rounded-xl border border-gray-200 bg-white shadow-sm">
        <div className="overflow-x-auto">
          <Table
            hoverable
            className="w-full min-w-240 text-left [&_.table-th]:font-semibold"
          >
      ...
      <ConfirmModal
        ...
        messages={{
          pending: {
            ...
            title: isBulkDelete ? "Delete Selected Products?" : "Are you sure?",
          ...
          success: {
            ...
            description: isBulkDelete
              ? `${selectedIds.length} products have been deleted.`
